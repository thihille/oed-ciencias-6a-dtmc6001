...
var dadosDeInstrucaoNavegacao = function(){
	if(localStorage.getItem('novaInstrucao') == 'sem instrucao'){
		...
		if(localStorage.getItem('novaInstrucao')){
	    	...
	    	$('.InstrucoesPrincipal').find('.texto_instrucoes').html(CONFIG[0].infoOed.instrucoes);
				localStorage.removeItem("novaInstrucao");
		}
	}
	if(localStorage.getItem('novaNavegacao') == 'sem navegacao'){
	    ...
	    $('.InstrucoesPrincipal').find('.texto_navegabilidade').html(CONFIG[0].infoOed.navegacao);
...
/* Lista de funções para animações e movimentos */
	function animacao_entrada_capa(){
		...
		if(CONFIG[0].infoOed.template == 'apoema'){
			$('.tituloCapa').hide();
			setTimeout(function(){
				$('.tituloCapa').show().addAnimateClass('bounceInDown animated', function(){
...
/* Organização de interações: click, over, out, etc. */
    ...
    $(".btn_menu, #tempo_btn_menu_fora").click(function(){
		status_menu++;
		if(status_menu>=2){
    ...
    $(".btn_instrucoes, #tempo_grupo_btn_instrucoes").click(function(){
